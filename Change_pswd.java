...
public class Change_pswd extends JFrame {
	...
	public Change_pswd() {
		...
		contentPane.setBackground(new Color(99, 0, 99));
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/folder lock", "root", "");
					PreparedStatement smt = con.prepareStatement("update lock_login set Password=? where username=?");
					smt.setString(1, pswd.toString());
